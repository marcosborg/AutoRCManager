        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.generalState.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.general-states.update", [$generalState->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.generalState.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $generalState->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('notification') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="notification" id="notification" value="1" {{ $generalState->notification || old('notification', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('emails') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="emails" id="emails" value="{{ old('emails', $generalState->emails) }}">
                        ...
                        <div class="form-group {{ $errors->has('message') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="message" id="message">{!! old('message', $generalState->message) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('position') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="position" id="position" value="{{ old('position', $generalState->position) }}" step="1">
