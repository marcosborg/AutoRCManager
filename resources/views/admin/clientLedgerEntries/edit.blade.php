        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.clientLedgerEntry.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.client-ledger-entries.update", [$clientLedgerEntry->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('client') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="client_id" id="client_id" required>
                                @foreach($clients as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('client_id') ? old('client_id') : $clientLedgerEntry->client->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('vehicle') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="vehicle_id" id="vehicle_id">
                                @foreach($vehicles as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vehicle_id') ? old('vehicle_id') : $clientLedgerEntry->vehicle->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('entry_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="entry_type" id="entry_type" required>
                                @foreach($entryTypes as $value => $label)
                                    <option value="{{ $value }}" {{ (old('entry_type') ? old('entry_type') : $clientLedgerEntry->entry_type) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="amount" id="amount" value="{{ old('amount', $clientLedgerEntry->amount) }}" step="0.01" min="0" required>
                        ...
                        <div class="form-group {{ $errors->has('entry_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="entry_date" id="entry_date" value="{{ old('entry_date', $clientLedgerEntry->entry_date) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $clientLedgerEntry->description) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('notes') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="notes" id="notes">{{ old('notes', $clientLedgerEntry->notes) }}</textarea>
                            @if($errors->has('notes'))
                        ...
                        <div class="form-group {{ $errors->has('attachment') ? 'has-error' : '' }}">
                            ...
                            @if($clientLedgerEntry->attachment->isNotEmpty())
                                <div style="margin-top: 5px;">
                                    @foreach($clientLedgerEntry->attachment as $media)
                                        <a href="{{ $media->getUrl() }}" target="_blank">{{ trans('global.view_file') }}</a>
                                    @endforeach
                                    <div class="checkbox" style="margin-top: 5px;">
                                        <label for="clear_attachment" style="font-weight: 400">
                                            <input type="checkbox" name="clear_attachment" id="clear_attachment" value="1">
                                            Remover anexo atual
                                        </label>
                                    </div>
                                </div>
