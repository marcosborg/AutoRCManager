...
function updateClientPaymentsList(payments) {
    ...
    payments.forEach((payment, index) => {
        paymentsList += `
            <li class="list-group-item">
                ${parseFloat(payment.client_invoice_amount).toFixed(2)}€ em ${payment.client_invoice_date}.
                ${payment.client_invoice_obs ? "(" + payment.client_invoice_obs + ")" : ""}
            </li>
        `;
    });
    ...
    // Atualiza o HTML no elemento com id `client_payments_container`
    $('#client_payments_container').html(paymentsList);
}
</script>
