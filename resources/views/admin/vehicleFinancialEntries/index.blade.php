...
<div class="content">
    @can('vehicle_financial_entry_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.vehicle-financial-entries.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.vehicleFinancialEntry.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('cruds.vehicleFinancialEntry.title_singular') }} {{ trans('global.list') }}
                </div>
                <div class="panel-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-VehicleFinancialEntry">
                            <thead>
                                <tr>
                                    <th width="10">
                                    </th>
                            ...
                            <tbody>
                                @foreach($vehicleFinancialEntries as $key => $entry)
                                    <tr data-entry-id="{{ $entry->id }}">
                                        ...
                                        <td>
                                            {{ $entry->entry_type === 'cost' ? 'Custo' : 'Receita' }}
                                        </td>
                                        <td>
                                            {{ $entry->category ?? '' }}
                                        </td>
...
@can('vehicle_financial_entry_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.vehicle-financial-entries.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-VehicleFinancialEntry:not(.ajaxTable)').DataTable({ buttons: dtButtons })
