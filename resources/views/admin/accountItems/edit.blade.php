        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.accountItem.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.account-items.update", [$accountItem->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.accountItem.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $accountItem->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('account_category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="account_category_id" id="account_category_id" required>
                                @foreach($account_categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('account_category_id') ? old('account_category_id') : $accountItem->account_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\AccountItem::TYPE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ old('type', $accountItem->type) === (string) $key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('total') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total" id="total" value="{{ old('total', $accountItem->total) }}" step="0.01">
                            @if($errors->has('total'))
