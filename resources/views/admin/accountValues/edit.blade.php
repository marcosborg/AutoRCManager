        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.accountValue.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.account-values.update", [$accountValue->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('account_item') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="account_item_id" id="account_item_id" required>
                                @foreach($account_items as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('account_item_id') ? old('account_item_id') : $accountValue->account_item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('value') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="value" id="value" value="{{ old('value', $accountValue->value) }}" step="0.01" required>
                            @if($errors->has('value'))
