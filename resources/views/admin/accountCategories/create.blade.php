        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.accountCategory.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.account-categories.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('account_department') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="account_department_id" id="account_department_id" required>
                                @foreach($account_departments as $id => $entry)
                                    <option value="{{ $id }}" {{ old('account_department_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
